...
describe("runCli", () => {
    it("prints the index confirmation line", async () => {
        const stdout = vi.fn();
        const stderr = vi.fn();
        const exitCode = await runCli(["node", "docsearch", "index", "--file", "docs/report.pdf"], {
            writeStdout: stdout,
            writeStderr: stderr,
    ...
    it("prints query results exactly in the required plain-text format", async () => {
        ...
        expect(exitCode).toBe(0);
        expect(stdout).toHaveBeenCalledWith('Results for: "What attention mechanism is proposed?"\n\n1. report.pdf  page 4\n2. report.pdf  page 7\n');
        expect(stderr).not.toHaveBeenCalled();
    });
    it("prints CLI failures to stderr and returns a non-zero exit code", async () => {
        const stdout = vi.fn();
        const stderr = vi.fn();
        const exitCode = await runCli(["node", "docsearch", "query", "--file", "docs/report.pdf", "--question", ""], {
            writeStdout: stdout,
            writeStderr: stderr,
            indexPdf: vi.fn(),
            queryPdf: vi.fn(async () => {
