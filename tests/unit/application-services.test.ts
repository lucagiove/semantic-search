...
import type { EmbeddingGenerator, PdfTextExtractor } from "../../src/domain/ports.js";
import {
  indexPdf,
  queryPdf
...
class FakePdfTextExtractor implements PdfTextExtractor {
  public constructor(private readonly pages: readonly { pageNumber: number; text: string }[]) {}

  public async extractPages(_filePath: string) {
    return this.pages;
  }
}

class FakeEmbeddingGenerator implements EmbeddingGenerator {
  public constructor(private readonly vectorsByText: ReadonlyMap<string, readonly number[]>) {}
  ...
  public async embedTexts(texts: readonly string[]) {
...
describe("indexPdf", () => {
  ...
  it("rejects empty file paths", async () => {
    ...
    await expect(
      ...
      new CliError("FILE_NOT_FOUND", "A PDF file path is required.")
    );
  });
});
...
describe("queryPdf", () => {
  ...
  it("rejects an empty question", async () => {
    ...
    await expect(
      queryPdf(
        { filePath: "docs/report.pdf", question: "   ", top: 5 },
        { pdfTextExtractor: extractor, embeddingGenerator: embeddings }
      )
    ).rejects.toEqual(
      new CliError("EMPTY_QUESTION", "The --question value must not be empty.")
    );
  });
  ...
  it("rejects non-positive top values", async () => {
    ...
    await expect(
      queryPdf(
        { filePath: "docs/report.pdf", question: "hello", top: 0 },
        { pdfTextExtractor: extractor, embeddingGenerator: embeddings }
      )
    ).rejects.toEqual(
      new CliError("INVALID_TOP_VALUE", "The --top value must be a positive integer.")
    );
  });
});
